...
function populateNotifications(response) {
    ...
    if (response.count == 0 || !response.data) {
        ...
        for (var i=0; i < response.count; i++) {
            $("#innerResults").append("<a data-context=\"notificationToRead\" data-role=\"button\" data-id=\"" + response.data[i]._id + "\"> Amount: " + response.data[i].bitconsAmount + " Rate: " + response.data[i].rate +"</a>");
        }
    }
